...
export const OrderUpload: React.FC<OrderUploadProps> = ({
  onUploadComplete
}) => {
  ...
  const loadDocuments = async () => {
    ...
    try {
      setLoading(true);
      const userDocs = await storageService.getUserTilausDocuments(user.uid);
      setDocuments(userDocs);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to load tilaus documents');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteDocument = async () => {
    ...
    try {
      setLoading(true);
      await storageService.deleteTilausDocuments(user.uid);
      setDocuments([]);
    } catch (err) {
  ...
  const downloadAsCSV = (document: ERPDocument) => {
    ...
    link.setAttribute('download', `${document.name.replace(/\.[^/.]+$/, '')}_tilaus.csv`);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      const uploadedDoc = await storageService.uploadTilausDocument(file, user.uid);
        ...
        {documents.length > 0 && (
          <>
            ...
            <div className="space-y-4">
              <h3 className="text-lg font-medium">Ladatut Tilaus Tiedostot</h3>
                      ...
                      <div className="flex items-center space-x-2">
                        <Dialog>
                          ...
                          <DialogContent className="sm:max-w-[800px] max-h-[600px]">
                            <DialogHeader>
                              <DialogTitle>Tilaus Preview: {doc.name}</DialogTitle>
                              <DialogDescription>
